...
// Encrypt group key for a member using ECDH shared secret
// memberPublicKeyB64: member's ECDH public key (SPKI format, base64)
// Returns: base64 encoded IV + encrypted key
function encryptGroupKeyForMember(groupKeyB64, memberPublicKeyB64) {
  // For simplicity, we use AES-GCM with a key derived from hashing the member's public key
  // In production, you'd derive a shared secret using ECDH
  const algorithm = 'aes-256-gcm';

  // Derive a key from the member's public key (simplified - in production use ECDH)
  const memberPubBuffer = base64ToBuffer(memberPublicKeyB64);
  const derivedKey = crypto.createHash('sha256').update(memberPubBuffer).digest();

  const iv = crypto.randomBytes(16);
  const cipher = crypto.createCipheriv(algorithm, derivedKey, iv);

  const encrypted = Buffer.concat([
    cipher.update(groupKeyB64, 'utf8'),
    cipher.final()
  ]);
...
// Decrypt group key using member's public key (for backend admin operations)
function decryptGroupKeyForMember(encryptedB64, memberPublicKeyB64) {
  const algorithm = 'aes-256-gcm';
